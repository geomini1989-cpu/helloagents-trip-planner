<template>
  ...
  <div v-if="tripPlan && sessionId" class="ai-chat-bar">
      <div class="chat-wrapper">
        <div class="chat-input-box">
          ...
          <input 
            ...
            :disabled="isRevising"
            type="text" 
            placeholder="💬 试试输入：'把第2天改成去环球影城'..."
            class="custom-chat-input"
          />
          <button 
            @click="handleRevise"
            :disabled="isRevising || !feedback"
            class="send-btn"
            :class="{ 'loading': isRevising }"
          >
            {{ isRevising ? '思考中...' : '调整行程' }}
          </button>
        </div>
...
let map: any = null
const sessionId = ref('')     // 会话 ID
const feedback = ref('')      // 用户反馈输入
const isRevising = ref(false) // 修改加载状态

onMounted(async () => {
  // 优先从 router state 获取 (最新生成的)
  const state = history.state
  
  if (state && state.plan) {
    tripPlan.value = state.plan
    if (state.sessionId) {
      sessionId.value = state.sessionId
    }
  } 
  // 如果 state 没数据，尝试从 sessionStorage 恢复 (页面刷新时)
  else {
    const cachedPlan = sessionStorage.getItem('tripPlan')
    const cachedSession = sessionStorage.getItem('sessionId')
    
    if (cachedPlan) {
      tripPlan.value = JSON.parse(cachedPlan)
    }
    if (cachedSession) {
      sessionId.value = cachedSession
    }
  }
...
const handleRevise = async () => {
  ...
  const hideLoading = message.loading('AI 正在重新规划行程...', 0)
  
  try {
    // 调用后端修改接口
    const res = await fetch('http://localhost:8000/api/trip/revise', {
      method: 'POST',
    ...
    if (data.data || data.plan) {
      const newPlan = data.data || data.plan
